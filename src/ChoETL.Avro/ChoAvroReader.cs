...
namespace ChoETL
{
    ...
    //where T : class
    {
        ...
        private IAvroReader<T> _avroReader;
        ...
        public ChoAvroReader(IAvroReader<T> parquetReader, ChoAvroRecordConfiguration configuration = null)
        {
            ChoGuard.ArgumentNotNull(parquetReader, "AvroReader");

            Configuration = configuration;
            Init();

            _avroReader = parquetReader;
        }
        ...
        public ChoAvroReader<T> Load(string filePath)
        {
            ChoGuard.ArgumentNotNullOrEmpty(filePath, "FilePath");
        ...
        public ChoAvroReader<T> Load(StreamReader sr)
        {
        ...
        public ChoAvroReader<T> Load(IAvroReader<T> parquetReader)
        {
            ChoGuard.ArgumentNotNull(parquetReader, "AvroReader");
        ...
        public ChoAvroReader<T> Load(Stream inStream)
        {
            ChoGuard.ArgumentNotNull(inStream, "Stream");
        ...
        private IAvroReader<T> Create(StreamReader sr)
        {
            if (Configuration != null)
            {
                if (Configuration.CodecFactory != null)
                    return AvroContainer.CreateReader<T>(sr.BaseStream, true, Configuration.AvroSerializerSettings, Configuration.CodecFactory);
                else
                    return AvroContainer.CreateReader<T>(sr.BaseStream, true, Configuration.AvroSerializerSettings, new CodecFactory());
            }
            else
                return AvroContainer.CreateReader<T>(sr.BaseStream, true);
        }
        ...
        public IEnumerator<T> GetEnumerator()
        {
            ...
            var e = rr.AsEnumerable<T>(_avroReader).GetEnumerator();
        ...
        public ChoAvroReader<T> IgnoreFieldValueMode(ChoIgnoreFieldValueMode mode)
        {
